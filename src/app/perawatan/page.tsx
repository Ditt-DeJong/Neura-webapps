...
export default function PerawatanKhusus() {
  // fixed: split color into separate static fields so Tailwind can scan them at build time
  // (dynamic class splitting like item.color.split(' ') breaks Tailwind's static analysis)
  const treatments = [
    {
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {treatments.map((item, idx) => (
              // fixed: use static class fields instead of runtime .split() — Tailwind can now scan these
              <div key={idx} className={`group border border-gray-100 p-10 rounded-3xl transition-all duration-300 hover:shadow-xl bg-white relative overflow-hidden ${item.hoverBorder}`}>
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 text-3xl ${item.iconClass}`}>
                  {item.icon}
                </div>
